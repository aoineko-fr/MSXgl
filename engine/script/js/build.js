...
if (DoCompile)
{
	...
	if (false)
	{
		...
		let rawFilesDefTxt = "// MSXgl | Raw files definitions generated by the Build tool\n";
		for (let i = 0; i < RawFiles.length; i++)
		{
			let raw = RawFiles[i];
			let name = path.parse(raw.file).base;
			name = name.replace(".", "_").toUpperCase();
			if (raw.offset !== undefined)
			{
				rawFilesDefTxt += `#define ${name}_ABS \t0x${util.getHex(raw.offset)}\n`; 
				rawFilesDefTxt += `#define ${name}_ADDR\t0x${util.getHex(raw.offset & 0xFFFF)}\n`; 
			}
			else if (raw.page !== undefined)
			{
				rawFilesDefTxt += `#define ${name}_ABS \t0x${util.getHex(raw.page * 16 * 1024)}\n`; 
				rawFilesDefTxt += `#define ${name}_PAGE\t${raw.page}\n`; 
				rawFilesDefTxt += `#define ${name}_ADDR\t0x${util.getHex((raw.page * 16 * 1024) & 0xFFFF)}\n`; 
			}
			else if (raw.segment !== undefined)
			{
				rawFilesDefTxt += `#define ${name}_ABS \t0x${util.getHex(raw.segment * SegSize)}\n`; 
				rawFilesDefTxt += `#define ${name}_SEG \t${raw.segment}\n`; 
				rawFilesDefTxt += `#define ${name}_ADDR\t0x${util.getHex((raw.segment * SegSize) & 0xFFFF)}\n`; 
			}
		}
		fs.writeFileSync("./rawfiles_def.h", rawFilesDefTxt);
		util.print("Success", PrintSuccess);
	}
