...
;  on 05/05/2022 at 13:34:30,09                                                         
;                                                                              
; Trilo Tracker and the Z80 players are coded and designed                     
...
decode_data_decode7::
    ...
    ld (0xe167), a
decode_data_decode8::
	ld hl, #PSG_registers-3
    dec (hl)
    jp nz, decode_data_decode_end
    ld a, (0xe18e)
    ld d, a
    ld a, (TRACK_Chan8+7)
    ld ix, #0xe195
...
skipd_skiph::
    ...
    dec a
    jp nz, skipd_no_new_step
    ld a, (replay_fade_vol)
    inc a
    cp #0x10
    jp c, skipd_cont_fadeing
    xor a
    ld (replay_fade), a
    jp replay_pause
skipd_cont_fadeing::
...
_replay_check_patternend::
    ...
    xor a
    cp (hl)
    jp nz, _replay_check_patternend_no_restart
    inc hl
    cp (hl)
    dec hl
    jp nz, _replay_check_patternend_no_restart
    ...
    ld h, (hl)
    ld l, a
_replay_check_patternend_no_restart::
	ld de, #TRACK_pointer1
    ld bc, #TRACK_cmd_E
    ldir
    ld (replay_orderpointer), hl
    jp decode_data_decode_end
decode_data_chan::
...
_replay_decode_ins::
    ...
    sub #0x71
    cp -17(ix)
    jp z, _replay_decode_ins_skip_ins
    ld -17(ix), a
    add a, a
    ld hl, (replay_insbase)
    add a, l
    ld l, a
    jp nc, _replay_decode_ins_skip
    inc h
_replay_decode_ins_skip::
	ld a, (hl)
    inc hl
    ld h, (hl)
    ld l, a
    ...
    inc hl
    cp -8(ix)
    jp z, _replay_decode_ins_skip_wav_trig
    set 6, d
    ld -8(ix), a
_replay_decode_ins_skip_wav_trig::
	.db 0xdd
    .db 0x75
    .db 0xf1
    .db 0xdd
...
_replaydecode_cmd::
    ...
    add a, l
    ld l, a
    jp nc, _replaydecode_cmd_skip
    inc h
_replaydecode_cmd_skip::
	ld a, (hl)
    inc hl
    ld h, (hl)
    ld l, a
...
decode_cmd3_port_tone_new_note::
    ...
    add a, l
    ld l, a
    jp nc, decode_cmd3_port_tone_new_note_skip
    inc h
decode_cmd3_port_tone_new_note_skip::
	ld e, (hl)
    inc hl
    ld d, (hl)
    ld l, +0(ix)
    ...
    add a, l
    ld l, a
    jp nc, decode_cmd3_port_tone_new_note_skip2
    inc h
    ccf 
decode_cmd3_port_tone_new_note_skip2::
	ld a, (hl)
    inc hl
    ld h, (hl)
    ld l, a
    ...
    ex af, af'
    bit 7, h
    jp nz, decode_cmd3_port_tone_new_note_skip3
    or #0x80
decode_cmd3_port_tone_new_note_skip3::
	ld +9(ix), a
    exx
    ret 
decode_cmd8_tremolo::
decode_cmd4_vibrato::
    ...
    ld e, a
    and #0xf
    jp z, decode_cmd4_vibrato_depth
    ld +12(ix), a
    neg
    ld +18(ix), a
decode_cmd4_vibrato_depth::
	ld a, e
    and #0xf0
    jp z, decode_cmd4_vibrato_end
    sub #0x10
    ld hl, #TRACK_Vibrato_sine
    add a, a
    jp nc, decode_cmd4_vibrato_skip1
    inc h
decode_cmd4_vibrato_skip1::
	add a, l
    ld l, a
    jp nc, decode_cmd4_vibrato_skip2
    inc h
decode_cmd4_vibrato_skip2::
	.db 0xdd
    .db 0x75
    .db 0xa
    .db 0xdd
...
decode_cmd17_track_detune::
    ...
    and #0x7
    bit 3, e
    jp z, decode_cmd17_track_detune_pos
    cpl 
    ld +4(ix), a
    ld +5(ix), #0xff
    jp _rdc
decode_cmd17_track_detune_pos::
	ld +4(ix), a
    ld +5(ix), #0x0
    jp _rdc
...
decode_cmd23_SCC_duty::
    ...
    add a, l
    ld l, a
    jp nc, decode_cmd23_SCC_duty_skip
    inc h
decode_cmd23_SCC_duty_skip::
	ld a, e
    ex af, af'
    inc a
    ld e, #0x77
decode_cmd23_SCC_duty_wspw_loop_h::
	ld (hl), e
    inc hl
    dec a
    jp nz, decode_cmd23_SCC_duty_wspw_loop_h
    ex af, af'
    sub #0x1f
    ld e, #0x87
decode_cmd23_SCC_duty_wspw_loop_l::
	ld (hl), e
    inc hl
    dec a
    jp nz, decode_cmd23_SCC_duty_wspw_loop_l
    jp _rdc
decode_cmd24_SCC_soften::
    ...
    add a, e
    ld e, a
    jp nc, decode_cmd24_SCC_soften_skip
    inc d
decode_cmd24_SCC_soften_skip::
	.db 0xfd
    .db 0x2e
    .db 0x20
decode_cmd24_SCC_soften_softloop::
	ld a, (hl)
    sra a
    ld (de), a
    .db 0xfd
    .db 0x2d
    jp z, decode_cmd24_SCC_soften_end
    inc hl
    inc de
    jp decode_cmd24_SCC_soften_softloop
decode_cmd24_SCC_soften_end::
	ex af, af'
    ld d, a
    jp _rdc_noinc
...
decode_cmd12_SCC_morph::
    ...
    and a
    jp z, decode_cmd12_SCC_morph_morph_fromtrack
decode_cmd12_SCC_morph_morph_fromregister::
	ld h, #0x98
    .db 0xfd
    .db 0x7c
    ld l, a
    jp decode_cmd12_SCC_morph_morph_copy
decode_cmd12_SCC_morph_morph_fromtrack::
	ld l, -8(ix)
    ld h, #0x0
    add hl, hl
    add hl, hl
    ...
    add hl, bc
decode_cmd12_SCC_morph_morph_copy::
    ...
    ex af, af'
    dec a
    jp nz, decode_cmd12_SCC_morph_morph_copy+5
    ld a, #0xff
...
process_data_chan::
    ...
    add a, l
    ld l, a
    jp nc, process_data_chan_skip
    inc h
process_data_chan_skip::
	ld a, (hl)
    inc hl
    ld h, (hl)
    ld l, a
...
macro_vol_add::
    ...
    add a, -3(ix)
    cp #0x10
    jp c, macro_vol_add_nolimit
    ld a, #0xf
macro_vol_add_nolimit::
	ld -3(ix), a
    jp _macro_set_volume
macro_vol_sub::
	ld a, (de)
    ...
    ld a, -3(ix)
    sub h
    jp nc, macro_vol_sub_nolimit
    xor a
macro_vol_sub_nolimit::
	ld -3(ix), a
_macro_set_volume::
	or -9(ix)
    ld b, -1(ix)
    sub b
    jp nc, _macro_set_volume_skip2
    add a, b
    and #0xf
_macro_set_volume_skip2::
	ld bc, (replay_mainvol)
    add a, c
    ld c, a
    jp nc, _macro_set_volume_skip
    inc b
_macro_set_volume_skip::
	ld a, (bc)
    bit 7, -7(ix)
    jp nz, _macro_set_volume_skip3
    rra 
    rra 
    rra 
    rra 
_macro_set_volume_skip3::
	and #0xf
    ld (SCC_regVOLE), a
_macro_end::
	.db 0xdd
...
process_cmd0_arpeggio::
	ld a, +17(ix)
    and a
    jp z, process_cmd0_arpeggio_nextNote
    dec a
    ld +17(ix), a
    ld a, +18(ix)
    and a
    jp z, process_cmd0_arpeggio_skip
    ld a, +6(ix)
    and #0xf
    ld +13(ix), a
    jp process_note
process_cmd0_arpeggio_skip::
	ld +13(ix), #0x0
    jp process_note
process_cmd0_arpeggio_nextNote::
	ld a, (replay_arp_speed)
    ld +17(ix), a
    ld a, +18(ix)
    and a
    jr nz, process_cmd0_arpeggio_skip1
    ld +18(ix), #0x1
    ld a, +6(ix)
    rlca 
    rlca 
    ...
    ld +13(ix), a
    jp process_note
process_cmd0_arpeggio_skip1::
	dec a
    jr nz, process_cmd0_arpeggio_skip2
    ld +18(ix), #0x2
    ld a, +6(ix)
    rlca 
    rlca 
    ...
    ld +6(ix), a
    and #0xf
    jp nz, process_cmd0_arpeggio_zero
    ld +18(ix), #0x0
process_cmd0_arpeggio_zero::
	ld +13(ix), a
    jp process_note
process_cmd0_arpeggio_skip2::
	ld +18(ix), #0x0
    ld +13(ix), #0x0
    jp process_note
...
process_cmd3_add::
	add a, l
    ld +0(ix), a
    jp nc, process_cmd3_add_skip
    inc h
process_cmd3_add_skip::
	bit 7, h
...
process_cmd8_tremolo::
    ...
    add a, l
    ld l, a
    jp nc, process_cmd8_tremolo_skip
    inc h
process_cmd8_tremolo_skip::
	ld a, (hl)
    add a, a
    add a, a
    add a, a
...
process_cmd4_vibrato::
    ...
    ld +18(ix), a
    bit 5, a
    jp z, process_cmd4_vibrato_pos
process_cmd4_vibrato_neg::
	and #0x1f
    add a, l
    ld l, a
    jp nc, process_cmd4_vibrato_skip1
    inc h
process_cmd4_vibrato_skip1::
	ld a, (hl)
    neg
    jp z, process_cmd4_vibrato_zero
    ld +2(ix), a
    ld +3(ix), #0xff
    jp process_note
process_cmd4_vibrato_pos::
	add a, l
    ld l, a
    jp nc, process_cmd4_vibrato_skip2
    inc h
process_cmd4_vibrato_skip2::
	ld a, (hl)
process_cmd4_vibrato_zero::
	ld +2(ix), a
    ld +3(ix), #0x0
    jp process_note
...
scc_reg_update_skip::
    ...
    dec hl
    dec c
    jp m, scc_reg_update_loop
    ret 
_write_SCC_wave::
	bit 0, a
    jp z, _write_SCC_wave_normalwave
    bit 1, a
    jp nz, _write_SCC_wave_sfxwave
_write_SCC_wave_ramwave::
	dec hl
    and #0xfe
    ld (hl), a
    ld hl, #_0x9800
    ...
    add a, l
    ld l, a
    jp nc, _write_SCC_wave_skip
    inc h
_write_SCC_wave_skip::
	jp copy_wave_fast
_write_SCC_wave_sfxwave::
	and #0xf8
    ld l, a
    ld h, #0x0
    add hl, hl
...
replay_process_morph::
    ...
    ld hl, #0xe0f2
replay_process_morph_loop::
	bit 4, (hl)
    jp z, replay_process_morph_skip
    set 6, (hl)
replay_process_morph_skip::
	add hl, de
    djnz replay_process_morph_loop
    inc a
...
SFX_WAVEBASE = .+2
    ...
    ld h, l
    ld h, h
    call po, _replaydecode_cmd_skip-4
    add a, b
    and b
    ret nz
    call z, 0xbcc0
...
TRACK_REC_SIZE = 0x27
TRACK_cmd_0 = 0x6
TRACK_cmd_A_add = 0x15
TRACK_cmd_E = 0x10
TRACK_cmd_ToneSlideAdd = 0x0
